...
import com.rls.ids.exceptions.AlreadyExistsException;
...
public class AppController {
    ...
    @RequestMapping(path = "company/signup", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) // Map
                                                                                                                       // ONLY
                                                                                                                       // POST
                                                                                                                       // Requests
    public ResponseEntity<CompanyResponseModel> companySignUp(@Valid @RequestBody Company company,
            @RequestHeader("SecretKey") String secretKey) throws NoSuchAlgorithmException {
        if (isInvalid(secretKey) || !secretKey.equals(this.secretKey)) {
            this.logger.info("SecretKey is missing or invalid.");
            throw new MissingHeaderException("SecretKey is missing or invalid.");
        }
        if (companyRepository.getCompanyByDomain(company.getDomain()) != null) {
            throw new AlreadyExistsException("The company with domain " + company.getDomain() + " already exists.");
        }
    ...
    @RequestMapping(path = "user/signup", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) // Map
                                                                                                                    // ONLY
                                                                                                                    // POST
                                                                                                                    // Requests
    public ResponseEntity<User> userSignUp(@Valid @RequestBody SignUpRequestModel signUpModel,
        ...
        if (admin == null) {
            throw new InvalidAppKeyException("AppKey " + signUpModel.getAppKey() + " is invalid!");
        }

        if (userRepository.getUserByCompanyIdAndUserId(admin.getCompanyId(), signUpModel.getUserId()) != null) {
            throw new AlreadyExistsException("The user with ID " + signUpModel.getUserId() + " already exists.");
        }
    ...
    public ResponseEntity<UserResolveResponseModel> resolveUser(@RequestHeader("AppKey") String appKey) {
        ...
        try {
            ...
            // List<java.util.Map.Entry> admin = (List<java.util.Map.Entry>)
            // adminList.get(0);
            User admin = userRepository.getUserByAppKey(appKey);
            ...
            throw new InvalidAppKeyException("AppKey " + appKey + " is invalid!" + e.toString());
        }
    }
